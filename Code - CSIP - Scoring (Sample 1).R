...
var.Ms <- colMeans(csip[,12:75], na.rm=T)
var.Ms
var.SD <- apply(csip[,12:75], 2, sd, na.rm=T)
var.SD

# Compute means and standard deviations for each participant
ind.Ms <- rowMeans(csip[,12:75], na.rm=T)
ind.Ms
ind.SD <- apply(csip[,12:75], 1, sd, na.rm=T)
ind.SD
which(ind.SD < .25)
...
csip.key <- list(CSIPPA=c("CSIP1","CSIP9","CSIP17","CSIP25","CSIP33","CSIP41","CSIP49","CSIP57"),
...
# Score from key
scores <- scoreItems(csip.key, csip, min=0, max=3, totals=T) # To use sums instead of means, use totals=T (but be careful w/ missingness)
...
scoresOut <- data.frame("UserID"=csip$UserID, "Gender"=csip$GENDER, scores$scores)
...
scoresOut2 <- data.frame("Gender"=csip$GENDER, scores$scores)
all <- scoresOut2 %>%
  split(.$Gender) %>%
  map(cor)

all[[2]][2:9, 2:9] # Toggle between "1" (Female) and "2" (Male)
...
# Create a scoring key
csip.key <- list("CSIPPA"=c(12,20,28,36,44,52,60,68),
                 "CSIPBC"=c(13,21,29,37,45,53,61,69),
                 "CSIPDE"=c(14,22,30,38,46,54,62,70),
                 "CSIPFG"=c(15,23,31,39,47,55,63,71),
                 "CSIPHI"=c(16,24,32,40,48,56,64,72),
                 "CSIPJK"=c(17,25,33,41,49,57,65,73),
                 "CSIPLM"=c(18,26,34,42,50,58,66,74),
                 "CSIPNO"=c(19,27,35,43,51,59,67,75))
# Score from key
scores <- scoreTest(csip, csip.key, nomiss=.80, minScore=0, maxScore=3) # nomiss indicates how much missingness is allowed
...
scoresOut <- data.frame("UserID"=csip$UserID, "Gender"=csip$GENDER, scores)
...
ind.desc <- describe(t(csip[,12:75]))
ind.desc

# Within-person standardize (ipsatize)
?ipsatize
zcsip <- ipsatize(csip[,12:75])
zcsip
...
csip.cent <- t(apply(csip[,12:75], 1, function(x) x - mean(x)))

# Using scoreItems from psych package to score ipsatized data
# Create a scoring key
csip.key <- list(CSIPPA.ip=c("CSIP1.ip","CSIP9.ip","CSIP17.ip","CSIP25.ip","CSIP33.ip","CSIP41.ip","CSIP49.ip","CSIP57.ip"),
...
# Score from key
zscores <- scoreItems(csip.key, zcsip, min=0, max=3)
...
zscoresOut <- data.frame("UserID"=csip$UserID, "Gender"=csip$GENDER, zscores$scores)
...
csip.out <- scoreTest(csip.key, csip.ip, rel=T, nomiss=6/8, totals=T)
describe(csip.out$scores)
csip.out$rel

# Rescaling for missingness
csip.out <- scoreTest(csip.key, csip.ip, rel=T, nomiss = 6/8)
csip.outX8 <- csip.out$scores * 8
describe(csip.outX8)
