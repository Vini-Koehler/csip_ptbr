...
NIC <- rowSums(cloit[,8:103], na.rm=T)
cloit$NIC <- rowSums(cloit[,8:103], na.rm=T)

#***********************************************************
#Assign variables in columns 2, 4, and 6 (see below) with a value of 2 when the item is endorsed
cloit$CLOIT17[cloit$CLOIT17 == 1] = 2.
cloit$CLOIT18[cloit$CLOIT18 == 1] = 2.
cloit$CLOIT19[cloit$CLOIT19 == 1] = 2.
cloit$CLOIT20[cloit$CLOIT20 == 1] = 2.
cloit$CLOIT21[cloit$CLOIT21 == 1] = 2.
cloit$CLOIT22[cloit$CLOIT22 == 1] = 2.
cloit$CLOIT23[cloit$CLOIT23 == 1] = 2.
cloit$CLOIT24[cloit$CLOIT24 == 1] = 2.
cloit$CLOIT25[cloit$CLOIT25 == 1] = 2.
cloit$CLOIT26[cloit$CLOIT26 == 1] = 2.
cloit$CLOIT27[cloit$CLOIT27 == 1] = 2.
cloit$CLOIT28[cloit$CLOIT28 == 1] = 2.
cloit$CLOIT29[cloit$CLOIT29 == 1] = 2.
cloit$CLOIT30[cloit$CLOIT30 == 1] = 2.
cloit$CLOIT31[cloit$CLOIT31 == 1] = 2.
cloit$CLOIT32[cloit$CLOIT32 == 1] = 2.

cloit$CLOIT49[cloit$CLOIT49 == 1] = 2.
cloit$CLOIT50[cloit$CLOIT50 == 1] = 2.
cloit$CLOIT51[cloit$CLOIT51 == 1] = 2.
cloit$CLOIT52[cloit$CLOIT52 == 1] = 2.
cloit$CLOIT53[cloit$CLOIT53 == 1] = 2.
cloit$CLOIT54[cloit$CLOIT54 == 1] = 2.
cloit$CLOIT55[cloit$CLOIT55 == 1] = 2.
cloit$CLOIT56[cloit$CLOIT56 == 1] = 2.
cloit$CLOIT57[cloit$CLOIT57 == 1] = 2.
cloit$CLOIT58[cloit$CLOIT58 == 1] = 2.
cloit$CLOIT59[cloit$CLOIT59 == 1] = 2.
cloit$CLOIT60[cloit$CLOIT60 == 1] = 2.
cloit$CLOIT61[cloit$CLOIT61 == 1] = 2.
cloit$CLOIT62[cloit$CLOIT62 == 1] = 2.
cloit$CLOIT63[cloit$CLOIT63 == 1] = 2.
cloit$CLOIT64[cloit$CLOIT64 == 1] = 2.

cloit$CLOIT81[cloit$CLOIT81 == 1] = 2.
cloit$CLOIT82[cloit$CLOIT82 == 1] = 2.
cloit$CLOIT83[cloit$CLOIT83 == 1] = 2.
cloit$CLOIT84[cloit$CLOIT84 == 1] = 2.
cloit$CLOIT85[cloit$CLOIT85 == 1] = 2.
cloit$CLOIT86[cloit$CLOIT86 == 1] = 2.
cloit$CLOIT87[cloit$CLOIT87 == 1] = 2.
cloit$CLOIT88[cloit$CLOIT88 == 1] = 2.
cloit$CLOIT89[cloit$CLOIT89 == 1] = 2.
cloit$CLOIT90[cloit$CLOIT90 == 1] = 2.
cloit$CLOIT91[cloit$CLOIT91 == 1] = 2.
cloit$CLOIT92[cloit$CLOIT92 == 1] = 2.
cloit$CLOIT93[cloit$CLOIT93 == 1] = 2.
cloit$CLOIT94[cloit$CLOIT94 == 1] = 2.
cloit$CLOIT95[cloit$CLOIT95 == 1] = 2.
cloit$CLOIT96[cloit$CLOIT96 == 1] = 2.

# Record "average intensity" by average values (do after assigning new values)
cloit$AIN <- rowSums(cloit[,8:103], na.rm=T)/NIC

names(cloit)
# Using scoreItems from psych package
?scoreItems
# Create a scoring key
cloit.key <- list(CLOITA=c("CLOIT1","CLOIT17","CLOIT33","CLOIT49","CLOIT65","CLOIT81"),
...
# Score from key
scores <- scoreItems(cloit.key, cloit, min=0, max=2, totals=T) # To use sums instead of means, use totals=T (but be careful w/ missingness)
...
scoresOut <- data.frame("UserID"=cloit$UserID, scores$scores)
...
subVars <- with(cloit, data.frame(UserID, NIC, AIN))
...
scoresOut2 <- data.frame("Gender"=cloit$GENDER, scores$scores)
all <- scoresOut2 %>%
  split(.$Gender) %>%
  map(cor)
...
# Create a scoring key
cloit.key <- list("CLOITA"=c(8,24,40,56,72,88),
...
# Score from key
scores <- scoreTest(cloit, cloit.key, nomiss=.80, minScore=0, maxScore=2) # nomiss indicates how much missingness is allowed
...
ind.desc <- describe(t(cloit[,15:78]))
ind.desc

# Within-person standardize (ipsatize)
?ipsatize
zcloit <- ipsatize(cloit[,8:103])
zcloit
...
cloit.cent <- t(apply(cloit[,8:103], 1, function(x) x - mean(x)))

# Using scoreItems from psych package to score ipsatized data
# Create a scoring key
cloit.key <- list(CLOITA.ip=c("CLOIT1.ip","CLOIT17.ip","CLOIT33.ip","CLOIT49.ip","CLOIT65.ip","CLOIT81.ip"),
...
# Score from key
zscores <- scoreItems(cloit.key, zcloit, min=0, max=3)
...
zscoresOut <- data.frame("UserID"=cloit$UserID, zscores$scores)
